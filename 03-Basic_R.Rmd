...
If a command is taking a long time to execute, you may want to cancel it.  To do this simply hit `Esc` and RStudio will give you back the ">" prompt (or `Ctrl+C` for a Mac). This is a useful tip to remember if you ever get stuck in a `+` loop in the console!

When using `R` as a calculator, the order of operations is the same as you would have learnt back in school (though **R** has a few additional operators to those you learned in school).
