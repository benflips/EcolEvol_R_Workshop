...
That said, there is no single best way to organise a file system. The key is to make sure that your chosen directory structure (and the location of the files therein) is consistent, informative, and, most importantly, that it works for you.

An example directory structure (for a `MyProject` root directory) might include the following:

+  A `data` folder that contains all input data (and associated metadata) for the analysis. 
...
+  An `output` folder to store any type of intermediate or output files. This could include model objects, simulations, etc. 
...
+  A title and "script abstract" at the top of the script saying what the script is for, who wrote it, and how it does it.
+  Use headings/subheadings to make navigating between sections easier. These will obviously depend on what the script is doing, but common ones might include `Load Packages`, `Load Data`, `Data Manipulation`, `Model Fitting`, `Plotting`, and `Save Outputs`.
...
+  Multi-line function calls. `R` recognises that a function call is not finished if the line ends in a comma, so you can add each argument in a function to a new line to make it fit on a page. For example,
```{r eval=FALSE}
sum(1:100,
    na.rm = TRUE)
```
